...
void findPath(const Matrix *matrix, Path **completePaths)
{
    ...
    // No need to malloc million bytes.
    //Logic is that untestedPaths is a Path* pointer for each row of matrix,
    // and then getNewPath allocates memory for each new path! 
    Path *untestedPaths = (Path*)malloc(sizeof(Path) * matrix->size.y);
    ...
    while (untestedPathSize > 0)
    {
     ...
     // Commented this free; as somtimes gets segmentation error/crash...need to see why!
    //free((Path*)untestedPaths);
}
...
int main(int argc, char *argv[])
{
    ...
    //This is O(m,3^n) algorithm; so instead of hardcoding for malloc, allocate correct memory!
    Path *completedPaths = (Path*)malloc(sizeof(Path) * matrix.size.y * pow(3, (matrix.size.x)));
