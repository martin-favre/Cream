...
// Depth first search
void findPath(const Matrix *matrix, Path **completePaths, int numOfPaths)
{
    //Path *untestedPaths = malloc(sizeof(Path) * 1000000);
    Path *untestedPaths = malloc(sizeof(Path) * numOfPaths);
...
int main(int argc, char *argv[])
{
    ...
    /* OWN CODE */
    int numOfPaths = pow(3, matrix.size.x) * matrix.size.y;
    //printf("%d\n", numOfPaths);


    // This will contain all resulting paths
    Path *completedPaths = malloc(sizeof(Path) * numOfPaths);
    //Path *completedPaths = malloc(sizeof(Path) * 1000000);
    findPath(&matrix, &completedPaths, numOfPaths);
