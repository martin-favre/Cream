...
int getLexiSmallestPath(Path *pathA, Path *pathB)
{
...
void findPath(const Matrix *matrix, Path *bestPath)
{
    ...
    while (untestedPathSize > 0)
    {
        ...
        {
        	int tmpScore = getPathScore(&currentPath);
        	if (tmpScore < currentMinValue) {
        		currentMinValue = tmpScore;
        		memcpy(&currentMinPath, &currentPath, sizeof(currentMinPath));
        	}
        	else if (tmpScore == currentMinValue && getLexiSmallestPath(&currentPath, &currentMinPath)) {
        		currentMinValue = tmpScore;
        		memcpy(&currentMinPath, &currentPath, sizeof(currentMinPath));
        	}
        	 //if equal -> lexigrahpic
     //       (*completePaths)[completePathSize] = currentPath;
     //       completePathSize++;
        }
        else
        {
        	if (getPathScore(&currentPath) > currentMinValue) {
        		continue;
        	}
