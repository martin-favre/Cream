...
{
	path->score += matrix->rows[path->position.y].values[path->position.x];
...
Path getNextPath(const Path *currentPath, const Matrix *matrix, char dir)
{
	...
	nextPath.score = currentPath->score;
...
void findPath(const Matrix *matrix, Path **completePaths)
{
    ...
    Path *untestedPaths = calloc(1000000, sizeof(Path));
    ...
    while (untestedPathSize > 0)
    {
        ...
        {
			if (currentPath.score < lowestScore)
			{
...
int getPathScore(const Path *path, int lowestScore)
{
	/*
    int sum = 0;
...
int main(int argc, char *argv[])
{
	...
	Path *completedPaths = calloc(1000000, sizeof(Path));
    ...
    for (int i = 0; i < completePathSize; i++)
    {
        ...
        int score = path.score; //getPathScore(&path, lowestScore);
        ...
        else if (score == lowest) //else if (score == lowest) //(score == lowestScore)
        {
